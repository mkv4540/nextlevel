...
function QuizResult() {
  ...
  const prizePool = parseFloat(searchParams.get("reward") || "0");
  ...
  useEffect(() => {
    const generateLeaderboard = () => {
      ...
      const rewardedPlayers = sortedPlayers.map((player, index) => {
        ...
        if (rank === 1) {
          playerReward = prizePool * 0.3;
        } else if (rank === 2) {
          playerReward = prizePool * 0.2;
        } else if (rank === 3) {
          playerReward = prizePool * 0.15;
        } else if (rank <= 10) {
          const remainingPool = prizePool * 0.25;
    ...
    const rewardedPlayers = generateLeaderboard();
    setLeaderboard(rewardedPlayers);
      ...
      <div className="max-w-3xl mx-auto p-6">
        <div className="bg-white rounded-lg shadow-md p-8 text-center mb-12">
          ...
          <div className="space-y-4 mb-8">
            ...
            <div>
              <p className="text-gray-600">Reward Earned</p>
              <p className="text-3xl font-bold text-green-600">₹{userReward}</p>
            </div>
          </div>
          <button
            onClick={() => router.push("/reward-quiz")}
